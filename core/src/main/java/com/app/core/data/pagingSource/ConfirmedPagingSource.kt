...
import com.app.core.data.repository.OrderHistoryRepository.Companion.START_PAGE_POSITION
import com.app.core.data.resource.Resource
import javax.inject.Inject

class ConfirmedPagingSource @Inject constructor(private val retrofitService: RetrofitService) :
  ...
  override suspend fun load(params: LoadParams<Int>): LoadResult<Int, OrdersResponseDTO> {
    val nextPageNumber = params.key ?: START_PAGE_POSITION
    return try {
      ...
      LoadResult.Page(
        data = dataList,
        prevKey = if (nextPageNumber == START_PAGE_POSITION) null else nextPageNumber.minus(1),
