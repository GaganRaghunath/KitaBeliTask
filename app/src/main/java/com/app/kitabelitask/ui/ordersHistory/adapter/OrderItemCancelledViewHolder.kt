...
class OrderItemCancelledViewHolder(private val _binding: AdapterOrderItemBinding) :
  ...
  override fun onBind(model: OrderHistoryResponse.OrderItem?) {
    ...
    _binding.statusTv.text = "Pengiriman dibatalkan"
    _binding.statusTv.backgroundTintList =
      ContextCompat.getColorStateList(_binding.root.context, R.color.colorCancelled)
    _binding.statusTv.setTextColor(
      ContextCompat.getColor(_binding.root.context, R.color.colorCancelled)
    )
